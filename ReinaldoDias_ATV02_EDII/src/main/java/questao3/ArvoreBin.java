 ...
 */
    ...
    private void Inserir_no(nodeAr aux, nodeAr novo){
        if(aux!=null){
            if(novo.getDado().compareTo(aux.getDado()) >= 0){
                if(aux.getDir()==null){
                    aux.setDir(novo);
                }else{
    ...
    public int retornaProfundidade_Min(){
        if(!Vazia_Arvore()){
            return ProfundidadeMinima(raiz);
        }
        return 0;
    }
    
    private int ProfundidadeMinima(nodeAr aux){
        if(aux==null){
            return Integer.MAX_VALUE;
        }else if(aux.getDir()==null && aux.getEsq()==null){
            return 1;
        }else{
            int direita =  ProfundidadeMinima(aux.getDir());
            int esquerda = ProfundidadeMinima(aux.getEsq());
            return Math.min(direita, esquerda) + 1;
        }
    }
    /**
     * Método para localizar uma subarvore e alterar seu dado
    ...
    private void emOrdem(nodeAr aux){
        if(aux != null){
            emOrdem(aux.getEsq());
            System.out.println(aux.getDado());
            emOrdem(aux.getDir());
    ...
    private void PosOrdem(nodeAr aux){
        if(aux != null){
            emOrdem(aux.getEsq());
            emOrdem(aux.getDir());
            System.out.println(aux.getDado());
